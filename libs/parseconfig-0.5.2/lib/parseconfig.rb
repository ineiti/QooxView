...
class ParseConfig
  ...
  # The raw add method, called from import_config
  def raw_add( group, param, value )
  ...
  def import_config()
    ...
    open(self.config_file).each do |line|
      line.strip!
      unless (/^\#/.match(line))
        if (/\s*=\s*/.match(line) and not config_spaces)
          #param, value = line.split(/\s*=\s*/, 2)
          raw_add( group, *line.split(/\s*=\s*/, 2 ) )
